...
from torch_geometric.utils import remove_self_loops, add_self_loops, subgraph, k_hop_subgraph
...
from dataset import load_dataset, get_split_indices
from data_utils import normalize, gen_normalized_adjs, eval_acc, to_sparse_tensor ,adj_mul
from eval import evaluate, evaluate_cpu, evaluate_CP, evaluate_CM
...
###
# Code Note:
# This Code is used for applying evaluating the performance on Cross-platforms performance
###
...
### Here we perform normalization and logtransformation to the scRNA-seq data ###
...
# c is inferer for the number of cell types
c = max(dataset.label.max().item() + 1, dataset.label.shape[1])
d = dataset.graph['node_feat'].shape[1]

if args.use_knn:
    e = dataset.graph['edge_index'].shape[1]
    # print(dataset.graph['edge_index'].device)
...
for run in range(args.runs):
    ...
    # train_loader = NeighborSampler(data.edge_index, node_idx=train_idx, sizes=[-1], batch_size=args.batch_size, shuffle=True)
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(num_batch):
            ...
            if args.use_knn:
                adjs_i = []
                # adj shape [2, N], edge_index.shape[0]=2
    ...
    ## output cross_platforms results 
    store_results = False
    if args.cross_platform:
        if store_results:
            with torch.no_grad():
                ...
                out = model(dataset.graph['node_feat'].to(torch.device("cpu")), dataset.graph['edge_index'].to(torch.device("cpu")))
                y_true = dataset.label[split_idx['test']].detach().cpu().numpy()
                y_pred = out[split_idx['test']].argmax(dim=-1, keepdim=True)
                stored_list['true'] = y_true
                stored_list['pred'] = y_pred
                # print(stored_list)
                store_path = "/home/xyfan/working/scProject/cache/umap_files/CP"
                true_pred_path = os.path.join(os.path.join(store_path, f'scGraphformer'), f'{args.dataset}_{args.query_dataset}_best_CP.pkl')
                # true_path = os.path.join(os.path.join(store_path, f'scGraphformer'), f'{args.dataset}_{args.query_dataset}_true.pkl')
                # pred_path = os.path.join(os.path.join(store_path, f'scGraphformer'), f'{args.dataset}_{args.query_dataset}_pred.pkl')  
                # with open(true_path, 'wb') as f:
                #     pkl.dump(y_true, f)
                with open(true_pred_path, 'wb') as f:
                    pkl.dump(stored_list, f)    
                print("True and pred files haven been saved.")
    confusion_matrix = False
    if confusion_matrix and args.runs==1:
        if not args.cross_platform:
            CM, test_pred, test_true = evaluate_CM(model, dataset, args)
        else:
            CM, test_pred, test_true = evaluate_CM(model, args.query_dataset, args)
        CM_path = "/home/xyfan/working/scProject/cache/ConfusionMatrix"
        if not args.cross_platform:
            CM_filename = f'{CM_path}/{args.dataset}/{args.dataset}_CM.npy'
        else:
            CM_filename = f'{CM_path}/{args.dataset}/{args.dataset}_{args.query_dataset}_CM.npy'
        if not os.path.exists(f'{CM_path}/{args.dataset}'):
            os.makedirs(f'{CM_path}/{args.dataset}')
        print(f"Saving Confusion Matrix results to {CM_filename}...")
        ...
        test['true'] = test_true
        if args.cross_platform:
            test_filename = f'{CM_path}/{args.dataset}/{args.dataset}_{args.query_dataset}_test.npy'
        else:
            test_filename = f'{CM_path}/{args.dataset}/{args.dataset}_test.npy'

        with open(test_filename, 'wb+') as f:
            np.save(f, test)
    
        # saving the split_idx
        if not args.cross_platform:
            split_path = os.path.join(os.path.join(CM_path, args.dataset), 'CM_split_idx_lst.pkl') 
            with open(split_path, 'wb') as f:
                pkl.dump(split_idx_lst, f)
        print("CMatrix and test files haven been saved.")
...
if args.get_attn:
    attn_folder = "/home/xyfan/working/scProject/cache/attention"
    if args.use_HVG:
